...
## Overview
This case study applies the NIST Cybersecurity Framework (CSF) to a distributed denial-of-service incident in which an unconfigured firewall allowed a flood of ICMP packets to overwhelm an organization's internal network for two hours. The report works through all five CSF core functions, Identify, Protect, Detect, Respond, and Recover.
...
- Structured the analysis around the CSF's subcategories (Asset/Technology, Access Control, Continuous Monitoring, Mitigation, Recovery Planning, etc.).
- Closed with prioritized, ownership-oriented recommendations, distinguishing an immediate low-cost fix (ICMP rate limiting, source IP verification) from a longer-term investment (IDS/IPS and SIEM deployment).
